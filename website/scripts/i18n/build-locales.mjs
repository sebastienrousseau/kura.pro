 ...
 *
 * Reads website/index.html as the EN template, extracts translatable strings,
 * and generates website/{lang}/index.html for each non-English language
 * using strings from translations.mjs.
 *
 * Run: node website/scripts/i18n/build-locales.mjs
 */
...
function loadTemplate() {
  return fs.readFileSync(path.join(WEBSITE, 'index.html'), 'utf8');
}
...
function main() {
  ...
  // First, inject the language switcher into the EN template itself
  const enT = TRANSLATIONS.en;
  const enHtml = render(template, 'en', enT);
  fs.writeFileSync(path.join(WEBSITE, 'index.html'), enHtml);
  console.log('✓ Updated EN: website/index.html');
  ...
  for (const lang of LANGUAGES) {
    ...
    const outDir = path.join(WEBSITE, lang.code);
    fs.mkdirSync(outDir, { recursive: true });
    fs.writeFileSync(path.join(outDir, 'index.html'), localized);
    console.log(`✓ Generated ${lang.code}: website/${lang.code}/index.html`);
  }

...
describe('Middleware: onRequest', () => {
  ...
  // --- Website pillar rewrites ---
  describe('website pillar rewrites', () => {
    it('rewrites / to /website/index.html', async () => {
      const ctx = makeContext('/');
      await onRequest(ctx);
      const fetchedUrl = ctx.env.ASSETS.fetch.mock.calls[0][0].url;
    ...
    it('rewrites /shared/ to /website/shared/', async () => {
      ...
      await onRequest(ctx);
      const fetchedUrl = ctx.env.ASSETS.fetch.mock.calls[0][0].url;
      expect(fetchedUrl).toContain('/website/shared/branding/akqa.svg');
    });
  });
  ...
  // --- Website pillar rewrites (extended) ---
  describe('website pillar rewrites (extended)', () => {
    it('rewrites /index.html to /website/index.html', async () => {
    ...
    it('rewrites /404.html to /website/404.html', async () => {
      const ctx = makeContext('/404.html');
      await onRequest(ctx);
      const fetchedUrl = ctx.env.ASSETS.fetch.mock.calls[0][0].url;
      expect(fetchedUrl).toContain('/website/404.html');
    });

    it('rewrites /robots.txt to /website/robots.txt', async () => {
      const ctx = makeContext('/robots.txt');
      await onRequest(ctx);
      const fetchedUrl = ctx.env.ASSETS.fetch.mock.calls[0][0].url;
      expect(fetchedUrl).toContain('/website/robots.txt');
    });

    it('rewrites /sitemap.xml to /website/sitemap.xml', async () => {
      const ctx = makeContext('/sitemap.xml');
      await onRequest(ctx);
      const fetchedUrl = ctx.env.ASSETS.fetch.mock.calls[0][0].url;
      expect(fetchedUrl).toContain('/website/sitemap.xml');
    });

    it('rewrites /api-reference to /website/api-reference/index.html', async () => {
      const ctx = makeContext('/api-reference');
      await onRequest(ctx);
      const fetchedUrl = ctx.env.ASSETS.fetch.mock.calls[0][0].url;
      expect(fetchedUrl).toContain('/website/api-reference/index.html');
    });

    it('rewrites /content/ to /website/content/', async () => {
      const ctx = makeContext('/content/docs/guide.html');
      await onRequest(ctx);
      const fetchedUrl = ctx.env.ASSETS.fetch.mock.calls[0][0].url;
      expect(fetchedUrl).toContain('/website/content/docs/guide.html');
    });
  });

...
```bash
cd website/scripts && npm install
...
```bash
node website/scripts/generate-manifest.mjs
...
```bash
CLOUDFLARE_API_TOKEN=<token> CLOUDFLARE_ACCOUNT_ID=<id> node website/scripts/sync-knowledge.mjs content
```

...
describe('main', () => {
  ...
  it('returns stats object with converted count', async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'convert-stats-'));
    await sharp({ create: { width: 5, height: 5, channels: 3, background: 'red' } })
      .png()
      .toFile(path.join(tmpDir, 'a.png'));
    await sharp({ create: { width: 5, height: 5, channels: 3, background: 'blue' } })
      .png()
      .toFile(path.join(tmpDir, 'b.png'));
  ...
  it('handles empty directory gracefully', async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'convert-empty-'));
    try {
      const stats = await main(tmpDir);
  ...
  it('handles directory with only non-image files', async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'convert-noimg-'));
